...
{ help file JCL.chm, available at the project jedi homepage.                   }
{ Portions created by these individuals are Copyright (C) of these individuals.}
{                                                                              }
{ Portions created by Thomas Werner are Copyright (C) 2004 Thomas Werner.      }
{                                                                              }
{******************************************************************************}
{                                                                              }
...
{                                                                              }
{ Last modified by Thomas Werner: September 2003                               }
{                                                                              }
{******************************************************************************}
...
// - Deleted an commented out version of StrReplace. If anyone ever want to
//   finish the old version please go the archive version 0.39
...
// - Note to Marcel:  Have a look at StrToStrings and StrItoStrings. They are
//                    untested but should work more or less equal to the
//                    BreakApart functions by JFC.
...
uses Classes, SysUtils, JclBase, JclUnicode;
...
function  StrReplaceChar(const S: AnsiString;
          const Source, Replace: Char): AnsiString;
...
//------------------------------------------------------------------------------
// Routines of Thomas Werner
//------------------------------------------------------------------------------

// converts a string (ansi) into a widestring (utf16)
function  Unicode(s: String): WideString;
// converts a widestring (utf16) into a string (ansi)
function  ANSI(w: WideString): String;
// conversion of pos for usage with widestrings
function  PosW(Substr,S: WideString): Integer;
// Finds the last position of sub in aString
function PosWRev(const aString: WideString; sub: WideChar): Integer;
// splits a string of seperated values into it's parts
procedure SplitW(const Str,Seperator: WideString; lst: TWideStrings);
// substitutes all sgml special characters in aString
// each character will be substituted by \XXXX whereby XXXX is the double byte
// hex code of the characters unicode value.
function  escape(const aString: WideString): WideString;
// reverses the substitution of all sgml special characters
function  unescape(const aString: WideString): WideString;

//------------------------------------------------------------------------------
// Exceptions
//------------------------------------------------------------------------------

type EJclStringError = EJclError;

implementation

uses {$IFDEF WIN32} Windows, {$ENDIF WIN32} JclLogic, JclResources;
...
begin
  ...
  begin
    GetStringTypeExA(LOCALE_USER_DEFAULT, CT_CTYPE1, @CurrChar,SizeOf(AnsiChar),
                     CurrType);
...
function StrIsAlpha(const S: AnsiString): Boolean;
  ...
  begin
    ...
    for I := 1 to Length(S) do begin
      if not CharIsAlpha(S[I]) then begin
        Result := False;
        Exit;
      end;
    end;
...
function StrIsAlphaNum(const S: AnsiString): Boolean;
  ...
  begin
    Result := S <> '';
    for I := 1 to Length(S) do begin
      if not CharIsAlphaNum(S[I]) then begin
        Result := False;
        Exit;
      end;
    end;
...
function StrIsAlphaNumUnderscore(const S: AnsiString): Boolean;
  ...
  begin
    for i := 1 to Length(s) do begin
      C := S[I];
      if not (CharIsAlphaNum(C) or (C = '_')) then begin
        Result := False;
        Exit;
      end;
    end;
...
function StrIsDigit(const S: AnsiString): Boolean;
  ...
  begin
    Result := S <> '';
    for I := 1 to Length(S) do begin
      if not CharIsDigit(S[I]) then begin
        Result := False;
        Exit;
      end;
    end;
...
function StrIsNumber(const S: AnsiString): Boolean;
  ...
  begin
    Result := S <> '';
    for I := 1 to Length(S) do begin
      if not CharIsNumber(S[I]) then begin
        Result := False;
        Exit;
      end;
    end;
...
function StrSame(const S1, S2: AnsiString): Boolean;
  begin Result := StrCompare(S1, S2) = 0; end;
...
function StrCharPosLower(const S: AnsiString; CharPos: Integer): AnsiString;
  begin
    ...
    if (CharPos > 0) and (CharPos <= Length(S)) then
      Result[CharPos] := uStrings.CharLower(Result[CharPos]);
  end;
...
function StrCharPosUpper(const S: AnsiString; CharPos: Integer): AnsiString;
  begin
    Result := S;
    if (CharPos > 0) and (CharPos <= Length(S)) then
      Result[CharPos] := uStrings.CharUpper(Result[CharPos]);
  end;

//------------------------------------------------------------------------------

function StrDoubleQuote(const S: AnsiString): AnsiString;
  begin Result := AnsiDoubleQuote + S + AnsiDoubleQuote; end;

//------------------------------------------------------------------------------

function StrEnsurePrefix(const Prefix, Text: AnsiString): AnsiString;
  var PrefixLen: Integer;
  begin
    PrefixLen := Length(Prefix);
    if Copy(Text, 1, PrefixLen) = Prefix then Result := Text
    else Result := Prefix + Text;
  end;

//------------------------------------------------------------------------------

function StrEnsureSuffix(const Suffix, Text: AnsiString): AnsiString;
  var SuffixLen: Integer;
  begin
    SuffixLen := Length(Suffix);
    if Copy(Text, Length(Text) - SuffixLen + 1, SuffixLen) = Suffix then
      Result := Text
    else Result := Text + Suffix;
  end;
...
begin
  ...
  while I <= Len do
  begin
    if not ((S[I] = '\') and (I < Len)) then Result := Result + S[I]
    else begin
      Inc(I); // Jump over escape character
      case S[I] of
        'a': Result := Result + AnsiBell;
        'b': Result := Result + AnsiBackspace;
        'f': Result := Result + AnsiFormFeed;
        'n': Result := Result + AnsiLineFeed;
        'r': Result := Result + AnsiCarriageReturn;
        't': Result := Result + AnsiTab;
        'v': Result := Result + AnsiVerticalTab;
        '\': Result := Result + '\';
        '"': Result := Result + '"';
        '''':Result := Result + ''''; // Optionally escaped
        '?': Result := Result + '?';  // Optionally escaped
        'x':
          if I < Len then HandleHexEscapeSeq                                    // Start of hex escape sequence
          else Result := Result + '\x';                                         // '\x' at end of AnsiString is not escape sequence
        '0'..'7': HandleOctEscapeSeq;                                           // start of octal escape sequence
      else Result := Result + '\' + S[I];                                       // no escape sequence
      end;
    end;
    Inc(I);
  end;
...
begin
  ...
  if (FromIndex <= 0) or (FromIndex > Length(Source)) or
     ...
     ((FromIndex + Count - 1) > Length(Source)) or ((ToIndex + Count - 1) >
     Length(Dest)) then
     Exit;
...
procedure StrReplaceCS(var S: AnsiString; const Search, Replace: AnsiString;
          Flags: TReplaceFlags);
var
...
procedure StrReplaceCI(var S: AnsiString; Search, Replace: AnsiString;
          Flags: TReplaceFlags);
var
  ResultStr: string;     { result string }
  SourcePtr: PChar;      { pointer into S of character under examination }
...
begin
  ...
  begin
    ...
    begin
      ...
      SourceMatchPtr := SourcePtr + 1;
      SearchMatchPtr := PChar(Search) + 1;
      while (UpCase(SourceMatchPtr^) = SearchMatchPtr^) and
            (SearchMatchPtr^ <> #0) do
      begin
        Inc(SourceMatchPtr);
        Inc(SearchMatchPtr);
...
function StrReplaceChars(const S: AnsiString;
         const Chars: TSysCharSet; Replace: Char): AnsiString;
var
  I: Integer;
begin
  Result := S;
...
function StrFind(const Substr, S: AnsiString;
         const Index: Integer): Integer; assembler;
const
   SearchChar: Byte = 0;
...
asm
        ...
        // bring the first char out of the Substr and point Substr to the next
        // char
...
@@Find:
        ...
        // does current char match primary search char? if not, go back to the
        // main loop
...
@@CompareNext:
        ...
        // dec loop counter and check if we reached the end. If yes then we
        // found it

        DEC     EDX
        JL      @@Found

        // get the chars from Str and Substr, if they are equal then continue
        // comparing
        ...
        // otherwise try the reverse case. If they still don't match go back to
        // the Find loop
...
function StrILastPos(const SubStr, S: AnsiString): Integer;
begin
  Result := uStrings.StrLastPos(StrUpper(SubStr), StrUpper(S));
end;
...
//------------------------------------------------------------------------------
// IMPORTANT NOTE: The StrMatch function does currently not work with the
// Asterix (*)

function StrMatch(const Substr, S: AnsiString; const Index: Integer
         ): Integer; assembler;
asm
...
function StrPrefixIndex(const S: AnsiString; const Prefixes: array of string
         ): Integer;
var
  I: Integer;
  Test: string;
...
function StrSearch(const Substr, S: AnsiString; const Index: Integer
         ): Integer; assembler;
asm
        // make sure that strings are not null
        ...
        // # of positions in Str to look at = Length(Str) - Length(Substr) -
        // Index - 2
...
@@Compare:
        ...
        // move # of char to compare into edx, edx will be our compare loop
        // counter.
...
@@StrIsNull:
        ...
        XOR     EAX, EAX                                                        // clear result and exit.
...
{ todoc
  ...
  unique: determines whether s can be added to the list if an entry already
          exists
...
function BooleanToStr(B: Boolean): AnsiString;
  const Bools: array [Boolean] of PChar = ('False', 'True');
  begin Result := Bools[B]; end;
...
function StrToIntSafe(const S: AnsiString): Integer;
  begin Result := Trunc(StrToFloatSafe(S)); end;
...
function  Unicode(s: String): WideString;
  var P: PWideChar;
  begin
    GetMem(P,Succ(Length(s))*2);
    StringToWideChar(s, P, Succ(Length(s)));
    setLength(Result,Length(s));
    copyMemory(Pointer(Result),P,Length(Result)*2);
    FreeMem(P);
  end;

function  ANSI(w: WideString): String;
  begin Result := WideCharToString(PWideChar(w)); end;

function  PosW(Substr,S: WideString): Integer;
  var r: PWideChar;
  begin
    r := StrPosW(PWideChar(s),PWideChar(SubStr));
    if r = nil then Result := 0
    else Result := ((Integer(r) - Integer(PWideChar(s))) div 2) +1;
  end;

function PosWRev(const aString: WideString; sub: WideChar): Integer;
  var p: PWideChar;
  begin
    p := StrRScanW(PWideChar(aString),sub);
    if p = nil then Result := -1
    else Result := ((Integer(Pointer(p)) -Integer(Pointer(aString))) div 2) +1;
  end;
...
procedure SplitW(const Str,Seperator: WideString; lst: TWideStrings);
  ...
  begin
    lst.Clear;
    s := PWideChar(Str);
    p := StrPosW(s,PWideChar(Seperator));
    while p <> nil do begin
      lst.Add(copy(s,0,((Integer(p) -Integer(s)) div 2)));
      s := Pointer(Integer(p) + 2 * Length(Seperator));
      p := StrPosW(s,PWideChar(Seperator));
    end;
    if Integer(s) < Integer(StrEndW(PWideChar(Str))) then
      lst.Add(copy(s,0,((Integer(StrEndW(PWideChar(Str))) -Integer(s)) div 2)));
  end;
...
function escape(const aString: WideString): WideString;
  ...
  begin
    ...
    for i := 1 to Length(aString) do begin
      f := False;
      for j := 0 to k-1 do
        if (aString[i] = subst[j]) or (ord(aString[i]) < 32) then begin
          Result := Result + '\' + Unicode(IntToHex(ord(aString[i]),4));
          f := True;
          break;
        end;
      if not f then Result := Result + aString[i];
    end;
  end;
...
function unescape(const aString: WideString): WideString;
  ...
  begin
    Result := aString;
    try
      if Length(aString) < 5 then exit
      else Result := '';
      i := 1;
      while i <= Length(aString) do begin
        if aString[i] = '\' then begin
          j := strToInt('$' +ANSI(copy(aString,i+1,4)));
          copyMemory(@w,@j,4);
          Result := Result + w;
          inc(i,5);
        end else begin
    ...
    except                                                                      // a empty string will be returned
    end;
  end;
